#!/bin/bash
# Linux-compatible version of SemarEnv loader
...
_semarenv_msg() {
  ...
  if (( $# == 0 )); then
    echo -e "${color}[SemarEnv]\033[0m"
  else
    echo -e "${color}[SemarEnv] ${*:1}\033[0m"
  fi
}
...
declare -a _semarenv_allowed_paths=()
_semarenv_config_hash=""

_semarenv_reload() {
  local config_file="$HOME/.config/SemarEnv/bin/.semarenv.dir"
  local -a valid_paths=()
  ...
  new_hash=$(_semarenv_hash "$config_file") || return 0

  [[ "$new_hash" == "$_semarenv_config_hash" ]] && return 0
