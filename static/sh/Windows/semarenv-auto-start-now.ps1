...
try {
  ...
  $exePath = "#EXECPATH#"
  $dataPath = "#DATAPATH#"
  $allowDir = Join-Path $env:ProgramData "SemarEnv"
  $allowFile = Join-Path $allowDir "semarenv.allowed-roots"
  ...
  try {
    $existingTask = $rootFolder.GetTask("semarenv-helper")
    if ($existingTask) {
      $rootFolder.DeleteTask($taskName, 1)
    }
  } catch {}
  ...
  $taskDefinition.RegistrationInfo.Description = "SemarEnv Helper Auto Start"
