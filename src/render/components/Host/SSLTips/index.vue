...
<script lang="ts" setup>
  ...
  const copyCommand = () => {
    ...
    const CARootFile = join(window.Server.BaseDir!, 'CA/SemarEnv-Root-CA.crt')
    let command = ''
    if (window.Server.isMacOS) {
      command = `cd "${CARoot}" && sudo -S security add-trusted-cert -d -r trustRoot -k "/Library/Keychains/System.keychain" "SemarEnv-Root-CA.crt"`
  ...
  const showFile = () => {
    const CARoot = join(window.Server.BaseDir!, 'CA/SemarEnv-Root-CA.crt')
